 ...
 *  AUTHOR:CMB
 *  DATE:2015/9/29
 *  FUNCTION:男士服装商品推荐（后台）
 */
namespace Home\Controller;
use Think\Controller;
class ManclothproductionController extends CommonController {
    ...
    public function add_handle(){
        ...
        if(!M('production')->add($data))
        {
            ...
            $this->success('添加成功',U('Manclothproduction/index'));
        }
    }
    ...
    public function delete_handle(){
        ...
        // 获取排在当前数据后面的元素
        $data         = M('production')->where("sort > %d AND type = '%s'",$current_data['sort'],'MANCLOTH')->select();
