...
class ShoeshopController extends CommonController {
    ...
    public function add_handle(){
        ...
        if(!M('shop')->add($data))
        {
            ...
            $this->success('添加成功',U('Shoeshop/index'));
        }
    }
        ...
        if($_POST['img'] == 1)
        {
            ...
            if($this->deleteFile($filename) == 1)
            {
                if(!M('shop')->data($data)->save()){
                    ...
                    $this->success('修改成功',U('Shoeshop/index'));
                }
            }else{
                echo "删除文件错误";
            }
    ...
    public function delete_handle(){
        ...
        // 获取排在当前数据后面的元素
        $data         = M('shop')->where("sort > %d AND type = '%s'",$current_data['sort'],'SHOE')->select();
