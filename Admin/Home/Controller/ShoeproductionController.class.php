...
class ShoeproductionController extends CommonController {
    ...
    public function add_handle(){
        ...
        if(!M('production')->add($data))
        {
            ...
            $this->success('添加成功',U('Shoeproduction/index'));
        }
    }
    ...
    public function delete_handle(){
        ...
        // 获取排在当前数据后面的元素
        $data         = M('production')->where("sort > %d AND type = '%s'",$current_data['sort'],'SHOE')->select();
